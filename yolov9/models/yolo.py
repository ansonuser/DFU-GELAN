...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        y, dt = [], []  # outputs
        for m in self.model:
            # print("current layer:", m)
            if m.f != -1:  # if not from previous layer
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
            if profile:
                self._profile_one_layer(m, x, dt)
            if isinstance(x, list):
                x = [j for j in x if j is not None]
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg=None, model=None, nc=1000, cutoff=10, dropout_rate=0.2, c_=1280):  # yaml, model, number of classes, cutoff index
        super().__init__()
    ...
    def _from_detection_model(self, model, nc=1000, cutoff=10):
        ...
        c = Classify(ch, nc, dropout_rate=self.dropout_rate, c_=self.c_)  # Classify()
