...
def attempt_load(weights, device=None, inplace=True, fuse=True):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        if isinstance(ckpt, dict) and ('model' in ckpt or 'ema' in ckpt):
            model_obj = (ckpt.get('ema') or ckpt['model']).to(device).float()
        else:
            backbone = Model(cfg="gelan-c.yaml")
            # ✅ Raw state_dict (weights only), assume ClassificationModel
            model_obj = ClassificationModel(model=backbone, nc=2, c_=256) 
            model_obj.load_state_dict(ckpt)
            model_obj.to(device).float()
        # Model compatibility updates
        if not hasattr(model_obj, 'stride'):
            model_obj.stride = torch.tensor([32.])
        if hasattr(model_obj, 'names') and isinstance(model_obj.names, (list, tuple)):
            model_obj.names = dict(enumerate(model_obj.names))  # convert to dict

        model.append(model_obj.fuse().eval() if fuse and hasattr(model_obj, 'fuse') else model_obj.eval())  # model in eval mode
