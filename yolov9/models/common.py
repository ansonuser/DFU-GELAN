    ...
    def __init__(self, weights='yolo.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            if hasattr(model, 'module') and hasattr(model.module, 'names'):
                ...
                names = {i: f'class_{i}' for i in range(getattr(model, 'nc', 2))}  # fallback
            # model.names = {i: 'class_label_here' for i in range(model.nc)}
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, c_=1280, dropout_rate=0.3):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()  
        self.conv = Conv(c1, c_, k, s, autopad(k, p), g)
        self.pool = nn.AdaptiveAvgPool2d(1)  # to x(b,c_,1,1)
        self.drop = nn.Dropout(p=dropout_rate, inplace=True)
